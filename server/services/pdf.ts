    ...
    try {
      ...
      if (organization.abn) {
        doc
          .fontSize(10)
          .fillColor("#444")
          .text(`ABN: ${organization.abn}`, { align: "left" });
      }
      doc.moveDown(0.5);
      doc
        .fontSize(18)
      ...
      if (customer.contact_name)
        doc.text(customer.contact_name);
      ...
      doc
        ...
        .fontSize(10)
        .text(
          `Date: ${createdAt.toLocaleDateString("en-AU")}`,
          rightX,
          doc.y
        );
      if (dueAt) {
        doc.text(
          `Due: ${dueAt.toLocaleDateString("en-AU")}`,
          rightX,
          doc.y
        );
      }
      ...
      if (
        ...
        doc.text(
          `Please use Invoice #${invoiceNumber} as payment reference.`
        );
      }
...
export async function generateInvoicePdf(
  ...
  try {
    ...
    // Fallback to PDFKit so Railway (and any env without libnspr4/etc.) still works
