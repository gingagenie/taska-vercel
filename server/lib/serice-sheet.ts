...
export async function generateServiceSheetPdfForCompletedJob(opts: {
  req: any;
  orgId: string;
  completedJobId: string;
}): Promise<{ pdfBuffer: Buffer; filename: string }> {
  const { req, orgId, completedJobId } = opts;

  if (!isUuid(completedJobId)) throw new Error("Invalid completedJobId");
