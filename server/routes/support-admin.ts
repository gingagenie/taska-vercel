...
router.get("/users", async (req, res) => {
  try {
    ...
    // Build WHERE conditions safely using Drizzle SQL
    ...
    if (searchPattern) {
      whereConditions.push(sql`(name ILIKE ${searchPattern} OR email ILIKE ${searchPattern})`);
    }
    ...
    if (status === 'active') {
      whereConditions.push(sql`is_active = true`);
    } else if (status === 'inactive') {
      whereConditions.push(sql`is_active = false`);
    }

    // Build WHERE clause
    const whereClause = whereConditions.length > 0 ? sql`WHERE ${sql.join(whereConditions, ' AND ')}` : sql``;

    // Count total records with safe parameterization
    const countResult: any = await db.execute(sql`
      SELECT COUNT(*) as total FROM support_users ${whereClause}
    `);
    const total = parseInt(countResult[0]?.total || 0);

    // Get paginated users with safe parameterization
    const usersResult: any = await db.execute(sql`
      SELECT 
        id, name, email, role, is_active, last_login_at, created_at,
        (SELECT COUNT(*) FROM support_audit_logs WHERE actor_id = support_users.id) as action_count
      FROM support_users 
      ${whereClause}
...
router.get("/users/:id", async (req, res) => {
  try {
    ...
    const userResult: any = await db.execute(sql`
      SELECT 
        id, name, email, role, is_active, last_login_at, created_at,
        (SELECT COUNT(*) FROM support_audit_logs WHERE actor_id = ${id}) as action_count,
        (SELECT COUNT(*) FROM support_tickets WHERE assigned_to = ${id}) as assigned_tickets_count,
        (SELECT MAX(created_at) FROM support_audit_logs WHERE actor_id = ${id}) as last_action_at
      FROM support_users 
      WHERE id = ${id}
    `);
    ...
    const recentActivityResult: any = await db.execute(sql`
      SELECT action, target, meta, created_at
...
router.get("/invites", async (req, res) => {
  try {
    const validationResult = invitesQuerySchema.safeParse(req.query);
    if (!validationResult.success) {
      return res.status(400).json({
        error: "Invalid query parameters",
        details: validationResult.error.flatten()
...
router.get("/audit", async (req, res) => {
  try {
    ...
    if (user_id) {
      whereConditions.push(sql`sal.actor_id = ${user_id}`);
    }
    
    if (actionPattern) {
      whereConditions.push(sql`sal.action ILIKE ${actionPattern}`);
    }
    
    if (start_date) {
      whereConditions.push(sql`sal.created_at >= ${start_date}`);
    }
    
    if (end_date) {
      whereConditions.push(sql`sal.created_at <= ${end_date}`);
    }
    ...
    // Count total records with safe parameterization
    const countResult: any = await db.execute(sql`
      SELECT COUNT(*) as total FROM support_audit_logs sal ${whereClause}
    `);
    const total = parseInt(countResult[0]?.total || 0);

    // Get audit logs with safe parameterization
  ...
  try {
    ...
    const userStatsResult: any = await db.execute(sql`
      SELECT 
        ...
        COUNT(*) FILTER (WHERE role = 'support_agent') as regular_users,
    ...
    const inviteStatsResult: any = await db.execute(sql`
      SELECT 
        ...
        0 as cancelled_invites,
        COUNT(*) FILTER (WHERE used_at IS NULL AND expires_at > now()) as pending_invites,
        COUNT(*) FILTER (WHERE expires_at < now() AND used_at IS NULL) as expired_invites
      FROM support_invites
    `);
