                ...
                {filtered.map((i:any)=>(
                  <tr key={i.id} className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-left">
                      <Link href={`/invoices/${i.id}`}><a className="font-medium hover:underline">{i.title}</a></Link>
                    </td>
                    <td className="px-3 py-2 text-center">{i.customer_name}</td>
                    <td className="px-3 py-2 text-center capitalize">{i.status}</td>
                    <td className="px-3 py-2 text-center">
                      <Link href={`/invoices/${i.id}/edit`}><a className="text-blue-600">Edit</a></Link>
                    </td>
                  </tr>
                ))}
                {filtered.length===0 && <tr><td colSpan={4} className="px-3 py-10 text-center text-gray-500">No invoices</td></tr>}
              </tbody>
            </table>
          </div>
        )}
