...
export default function PortalEquipmentList() {
  ...
  useEffect(() => {
    (async () => {
      const r = await fetch("/api/portal/equipment");
  ...
  const filtered = useMemo(() => {
    ...
    return equipment.filter((e) =>
      [e.name, e.serial_number, e.asset_number].filter(Boolean).some((v) => String(v).toLowerCase().includes(s))
    );
  }, [equipment, q]);
  ...
  return (
    <div className="p-4 sm:p-6 min-h-screen bg-gray-100 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map((e) => (
          <Link key={e.id} href={`/portal/equipment/${e.id}`}>
